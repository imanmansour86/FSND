...
def create_app(test_config=None):
  ...
  app = Flask(__name__)
  setup_db(app)
  cors = CORS(app, resources={'/*': {"origins": "*", "supports_credentials": True}}) # Set up CORS. Allow '*' for all origins. 
  ...
  def delete_question(question_id):
    ...
    try:
      ...
      question.delete()
      selection = Question.query.order_by(Question.id).all()
      paginate_questions(request, selection)  # paginate based on current location
  ...
  def new_question():
      ...
      try:
        if search:
          ...
          question = body.get('question', None) #if any does not exist we'll set to none 
  ...
  def get_questionsbycategory(type_id):
    current_categoryquestions = Question.query.filter(Question.category == type_id).all()
    
     # get all questions and paginate
    current_questions = paginate_questions(request, current_categoryquestions)
    current_category = Category.query.filter(Category.id == type_id).one_or_none()
